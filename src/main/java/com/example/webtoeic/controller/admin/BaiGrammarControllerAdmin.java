...
//import com.example.webtoeic.DTO.ByteArrayMultipartFile;
...
public class BaiGrammarControllerAdmin {
    ...
    public ResponseEntity<?> createBaiGrammar(
        ...
        try {
            ...
            baiGrammar.setAnhBaiGrammar(BASE_UPLOAD_IMAGE_URL + fileName);
            baiGrammar.setTenBaiGrammar(tenBaiGrammar);
            baiGrammar.setContentHTML(docContentAsHtml);
            baiGrammar.setContentMarkDown(joinedImageNames);

            BaseResponse response = baiGrammarService.save(baiGrammar);
    ...
    public ResponseEntity<?> updateBaiGrammar(
            @PathVariable int id,
            @RequestParam("file") MultipartFile file,
            @RequestParam("tenBaiGrammar") String tenBaiGrammar,
        ...
        BaseResponse fetchedBaiGrammar = baiGrammarService.getBaiGrammar(id);
        if (fetchedBaiGrammar.getStatus() != 200) {
            return new ResponseEntity<>(fetchedBaiGrammar.getMessage(), HttpStatus.NOT_FOUND);
        }
        ...
        try {
            ...
            BaseResponse updateResponse = baiGrammarService.update(id, baiGrammar);
    ...
    public ResponseEntity<?> deleteBaiGrammar(@PathVariable int id) {
        BaseResponse deleteResponse = baiGrammarService.delete(id);
